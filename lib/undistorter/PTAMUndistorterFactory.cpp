...
namespace libvideoio
{
	...
	OpenCVUndistorter *PTAMUndistorterFactory::loadFromJSON(const nlohmann::json &json, const std::shared_ptr<Undistorter> & wrap )
	{
		...
		if( json["calib_type"]!= "PTAM" ) return nullptr;


		//
		float *remapX = nullptr, *remapY = nullptr;

		array<float,5> inputCalibration, outputCalibration;
		array<int,2> outSize, inSize;


		if( json.count("input_calibration") != 1 ) return nullptr;
		inputCalibration = json["input_calibration"];

		if( json.count("input_size") != 1 ) return nullptr;
		inSize = json["input_size"];

		if( json.count("output_size") != 1 ) return nullptr;
		outSize = json["output_size"];

		string outputMode;
		if( json.count("output_mode") > 0 ) {
			outputMode = json.count("output_mode");
			...
			if( outputMode == "crop" ) {
				...
				LOG(INFO) << "NO RECTIFICATION";
			} else {
				LOG(INFO) << "Do not understand output mode \"" << outputMode << "\"";
				return nullptr;
			}

		} else {
			if( json.count("output_calibration") != 1 ) return nullptr;
			outputCalibration = json["output_calibration"];
		}
		...
		// Convenience substitutions for now...
		const int in_width = inSize[0];
		const int in_height = inSize[1];
		const int out_width = outSize[0];
		const int out_height = outSize[1];
		...
		const double xfactor = in_width / (1.0 * in_width);
		const double yfactor = in_height / (1.0 * in_height);
		fx = fx * xfactor;
		fy = fy * yfactor;
